...
def interview_step(user_interview: Interview, bot: Bot, update: Update):
    ...
    elif user_interview.step == InterviewStep.fullname.value:
        step_fullname(user_interview, bot, update)
...
def finish_test(user_interview: Interview, bot: Bot):
    bot.send_message(
        chat_id=user_interview.chat_id,
        text=f"Вы прошли тестирование и набрали {user_interview.questing_balls} баллов",
        parse_mode=ParseMode.HTML,
        reply_markup=ReplyKeyboardRemove()
    )

    user_interview.test_try_count = user_interview.test_try_count + 1
    if user_interview.questing_balls >= 17:
        user_interview.save_test_result_to_table()
        bot.send_message(
            chat_id=user_interview.chat_id,
            text=f"Поздравляем, тестирование пройдено успешно",
            parse_mode=ParseMode.HTML,
            reply_markup=ReplyKeyboardRemove()
        )
    else:
        if user_interview.test_try_count == 1:
            bot.send_message(
                chat_id=user_interview.chat_id,
                text=f"Вы можете пройти тестирование повторно",
                parse_mode=ParseMode.HTML,
                reply_markup=ReplyKeyboardRemove()
            )
        else:
            bot.send_message(
                chat_id=user_interview.chat_id,
                text=f"Вы дважды неуспешно прошли тестирование",
                parse_mode=ParseMode.HTML,
                reply_markup=ReplyKeyboardRemove()
            )
...
def test_step(user_interview: Interview, bot: Bot, text: str = ''):
    ...
    if finish_time.timestamp() < datetime.now().timestamp():
        ...
        bot.send_message(
            chat_id=user_interview.chat_id,
            text=f"Время выполнения теста закончилось",
            parse_mode=ParseMode.HTML,
            reply_markup=ReplyKeyboardRemove()
        )
        if user_interview.test_try_count == 1:
    ...
    if user_interview.questing_step == -1:
        message_text += 'Добрый день! \n' \
                        'Тест необходимо закончить до ' + finish_time.strftime('%Y-%m-%d %H:%M') + '\n\n'
        user_interview.questing_step += 1
    ...
    if question.try_count >= question.answer_count:
        ...
        if user_interview.questing_step >= len(question_list):
            finish_test(user_interview, bot)
            return
...
def step_start(user_interview: Interview, bot: Bot):
    ...
    with open('uploads/MCVP.png', 'rb') as photo:
        bot.send_photo(
            user_interview.chat_id,
            photo,
            caption=text,
            reply_markup=ReplyKeyboardMarkup(keyboard_markup),
            parse_mode=ParseMode.HTML
...
def step_start_2(user_interview: Interview, bot: Bot, update: Update):
    ...
    if text == 'Даю согласие на обработку и использование персональных данных':
        message_text = "Спасибо!\n\n " \
                       ...
                       "Обращаем Ваше внимание, что ТРУДОУСТРОЙСТВО в образовательную организацию на должность «Советник директора по воспитанию и взаимодействию с детскими общественными объединениями» возможно только при наличии высшего образования и (или) повышения квалификации/профессиональной переподготовки (по ПЕДАГОГИЧЕСКОМУ профилю).\n\n" \
...
def step_start_end(user_interview: Interview, bot: Bot, update: Update):
    ...
    if text == 'Старт':
        ...
        message_text = 'Введите Ваши Фамилию, Имя, Отчество'
        bot.send_message(
            chat_id=user_interview.chat_id,
            text=message_text,
            parse_mode=ParseMode.HTML,
        ...
        user_interview.update_step(InterviewStep.fullname)


def step_fullname(user_interview: Interview, bot: Bot, update: Update):
    text = update.message.text.encode('utf-8').decode()
    interview_answers = dict(user_interview.interview_answers)
    interview_answers['fullname'] = text
    user_interview.interview_answers = interview_answers
    user_interview.update_step(InterviewStep.ed_oo_work)
    message_text = 'Пожалуйста, выберите образовательную организацию из списка, в котором' \
...
def step_add_adm_okr(user_interview: Interview, bot: Bot, update: Update):
    ...
    if text == 'Добавить административный округ':
        ...
        finish_time = user_interview.start_bot_datetime + timedelta(days=5)

        message_text = 'Спасибо! Ваши данные отправлены на модерацию.\n\n ' + finish_time.strftime(
            '%d.%m.%Y %H:%M') + '' \
                                'Теперь Вам доступно прохождение теста. Обратите внимание, тест необходимо пройти до \n ' \
                                'Тест состоит из 32 вопросов. Вопросы могут быть как с единственным правильным, так и с множественным ответом. Время прохождения теста – 60 минут.\n' \
                                'Для прохождения теста дается две попытки. Для успешного прохождения надо ответить правильно минимум на 17 вопросов'
        keyboard_markup = [[
            KeyboardButton(text='Начать тестирование'),
        ]]
        bot.send_message(
